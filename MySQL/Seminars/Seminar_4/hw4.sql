...
SELECT COUNT(*) AS 'Количество пользователей', c.id
FROM users_communities AS uc LEFT JOIN communities AS c ON uc.community_id = c.id
...
JOIN
messages m
WHERE m.from_user_id = u.id AND m.to_user_id = 3 
...
JOIN
profiles AS p 
WHERE p.user_id = l.user_id AND TIMESTAMPDIFF(YEAR, p.birthday, NOW()) < 18
;
