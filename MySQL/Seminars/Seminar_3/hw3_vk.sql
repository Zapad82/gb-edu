...
SELECT COUNT(gender) birthday
FROM profiles
WHERE gender='m' AND DATEDIFF(NOW(), birthday); 

-- 3. Сколько заявок в друзья в каждом статусе? (таблица friend_requests) [GROUP BY]
SELECT status AS 'Статус заявок в друзья', COUNT(status) AS 'Количество'
FROM friend_requests
GROUP BY status;

-- 4.* Выведите номер пользователя, который отправил больше всех заявок в друзья (таблица friend_requests) [LIMIT].
SELECT initiator_user_id AS 'Номер пользователя, который отправил больше всех заявок в друзья'
FROM friend_requests
