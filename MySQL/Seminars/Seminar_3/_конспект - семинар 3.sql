...
WHERE user_id = 1
  AND media_type_id = 1;
-- 1 = id фоток в таблице media_types
-- Выбираем видео-новости указанного пользователя
select *
from media
where user_id = 1
  and media_type_id = 3 -- Выбираем видео-новости указанного пользователя
select filename
from media
where user_id = 1
  and (
  ...
  )  если убрать скобки перед
  OR,
  то получим записи других пользователей -- Агрегирующие функции (AVG, MAX, MIN, COUNT, SUM) можно применять к выборкам и без группировки (group by), в этом случае вся выборка будет считаться за 1 группу
...
select count(*),
  media_type_id
from media
group by media_type_id -- сколько новостей у каждого пользователя?
...
from media
group by user_id
having count(*) > 1 -- Выбираем друзей пользователя (сначала все заявки, в том числе не подтвержденные) 
SELECT *
...
union
4, 5, 6);
 На следующем уроке добавим вычисление user_id динамически (вложенным запросом) -- Выбираем сообщения от пользователя и к пользователю (мои и ко мне)
SELECT *
FROM messages
...
where from_user_id = 1
  ...
  or from_user_id = 2
  and to_user_id = 1
order by created_at desc -- Непрочитанные сообщения
...
FROM messages
WHERE to_user_id = 1
  AND is_read = 0 -- отметим прочитанными некоторые (определенный диалог)
